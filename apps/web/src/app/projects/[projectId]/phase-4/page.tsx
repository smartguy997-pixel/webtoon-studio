...
${context}
대본/연출 작가(agent_script)가 작성한 대본 요약: ${scriptSummary}

역할:
- 대본/연출 작가의 30컷 연출 의도를 존중하면서, 캐릭터 외형 일관성 위반 가능성이 높은 컷을 지적합니다
...
export default function Phase4Page({ params }: { params: { projectId: string } }) {
  ...
  const [epRoadmapInfo, setEpRoadmapInfo] = useState<{ title: string; event: string; tension: number; cliffhanger: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // Phase 3에서 선택한 시작 화 자동 로드
    const startEp = localStorage.getItem(`wts_phase3_start_ep_${projectId}`);
    if (startEp) setSelectedEp(parseInt(startEp) || 1);
  ...
  // Phase 3 로드맵에서 현재 화 정보 로드
  useEffect(() => {
    try {
      const p3 = JSON.parse(localStorage.getItem(`wts_phase3_${projectId}`) ?? "null");
      const allEps = (p3?.episodeCards ?? []).flatMap((ec: { episodes: { ep: number; title: string; event: string; tension: number; cliffhanger: string }[] }) => ec.episodes);
      const ep = allEps.find((e: { ep: number }) => e.ep === selectedEp);
      setEpRoadmapInfo(ep ? { title: ep.title, event: ep.event, tension: ep.tension, cliffhanger: ep.cliffhanger } : null);
    } catch { setEpRoadmapInfo(null); }
  ...
  const startScript = useCallback(async () => {
    ...
    try {
      // ── Step 1. Script writer: intro + 30-cut script ──
      setCurrentStep(1);
      await streamText(
        "script",
        `당신은 AI Webtoon Studio 대본/연출 작가(agent_script)입니다. ${selectedEp}화 대본 작성을 시작합니다.
Phase 2 세계관·캐릭터 시트, Phase 3 로드맵을 기반으로 이번 화의 핵심 서사 방향을 25~35자로 예고하세요.
말투: 창의적이고 간결하게.`,
      ...
      // ── Step 2. Character: SCC verification ──
      setCurrentStep(2);
      await streamText(
        "character",
        buildCharacterCheckPrompt(selectedEp, context, scriptSummary),
        [{ role: "user", content: "대본/연출 작가의 30컷 대본 SCC 검증 결과를 보고해주세요." }],
        apiKey,
      );
      ...
      {stage === "idle" && (
        <div className={s.idleWrap}>
          ...
          <div className={s.idleCard}>
            ...
            {epRoadmapInfo ? (
              <div style={{ width: "100%", background: "rgba(124,108,252,0.06)", border: "1px solid rgba(124,108,252,0.2)", borderRadius: 10, padding: "12px 14px", textAlign: "left" }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: "#f1f5f9", marginBottom: 4 }}>{epRoadmapInfo.title}</div>
                <div style={{ fontSize: 12, color: "#94a3b8", lineHeight: 1.6 }}>{epRoadmapInfo.event}</div>
                {epRoadmapInfo.cliffhanger && (
                  <div style={{ fontSize: 11, color: "#fbbf24", marginTop: 6 }}>⚡ {epRoadmapInfo.cliffhanger}</div>
                )}
              </div>
            ) : (
