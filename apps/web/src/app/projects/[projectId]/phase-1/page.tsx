...
export default function Phase1Page() {
  ...
  const runDebate = useCallback(async (g: string, c: string, plat: string, ep: string) => {
    ...
    // ── REAL API MODE (single system prompt loop) ──
    setDebatePhase("running");
    setTurnCount(1);

    // Conversation history sent to Claude each round
    const convHistory: Array<{ role: "user" | "assistant"; content: string }> = [];

    const firstUserMsg =
      `기획 분석을 시작해줘.\n장르: ${g} | 플랫폼: ${platLabel} | 목표화수: ${ep}\n기획: ${c.slice(0, 500)}`;

    convHistory.push({ role: "user", content: firstUserMsg });
    ...
    for (let round = 1; round <= MAX_ROUNDS; round++) {
      ...
      // Pick up user message typed while waiting
      const pendingMsg = pendingUserMsgRef.current;
      if (pendingMsg) {
        addMsg("user", round, pendingMsg, false);
        // Inject as assistant turn so Claude sees it naturally
        convHistory.push({ role: "assistant", content: `[사용자]: ${pendingMsg}` });
        convHistory.push({ role: "user", content: "계속 토론해줘." });
        pendingUserMsgRef.current = null;
      }

      // Stream one round — Claude returns 1~3 agent lines
      let roundText = "";
      const roundMsgIds: Map<AgentId, string> = new Map();
      ...
      for await (const chunk of streamClaude({
        ...
        // Parse and display incrementally as lines arrive
        const parsed = parseAgentMessages(roundText);
        for (const { agentId, text } of parsed) {
          if (!roundMsgIds.has(agentId)) {
            const id = addMsg(agentId, round, text, true);
            roundMsgIds.set(agentId, id);
          } else {
      ...
      for (const [agentId, id] of roundMsgIds) {
        const final = parseAgentMessages(roundText).find(m => m.agentId === agentId);
        updateMsg(id, final?.text ?? "", false);
      }

      // Add to conversation history
      convHistory.push({ role: "assistant", content: roundText });
      convHistory.push({ role: "user", content: "계속해줘." });

      await sleep(800);
    }

    // Final report by producer
    await sleep(1000);
    setTurnCount(MAX_ROUNDS + 1);
    setIsWritingReport(true);

    const reportHistory = [
      ...convHistory,
      { role: "user" as const, content: buildFinalReportPrompt(
        convHistory.filter(m => m.role === "assistant").map(m => m.content).join("\n\n")
      )},
    ];

    let reportText = "";
    const reportId = addMsg("producer", MAX_ROUNDS + 1, "", true);
