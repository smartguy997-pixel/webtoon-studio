...
interface Episode {
  ...
  summary: string;
  tension: number;
  flags: Array<"hook" | "peak" | "reversal" | "special">;
  arc: number;
}
...
function buildMock(genre: string, title: string): Phase3Data {
  ...
  const arcs: Arc[] = names.map((name, i) => ({
    num: i + 1, name,
    theme: arcThemes[i],
    eps: [[1,25],[26,50],[51,75],[76,100]][i] as [number,number],
    color: ARC_COLORS[i],
  }));
  ...
  const episodes: Episode[] = Array.from({ length: 100 }, (_, i) => {
    ...
    const tension = Math.min(5, Math.max(1,
      Math.round(2 + (ep / 100) * 2.5 + Math.sin(ep * 0.3) * 0.8)
    ));
    ...
    return {
      ep,
      title: `${ep}화 — ${base}${arc >= 3 ? "의 끝" : ""}`,
      summary: `${arc}막 ${ep - [0,0,25,50,75][arc]}화차. ${base} 국면이 펼쳐지며 ${tension >= 4 ? "긴장감이 최고조에 달한다" : "이야기가 진행된다"}.`,
      tension, flags, arc,
    };
  });
    ...
    <div className={s.tensionBar}>
      {[1,2,3,4,5].map(n => (
        <div key={n} className={s.tensionDot}
          style={{ background: n <= level ? colors[level] : "var(--surface-3)" }} />
      ))}
    </div>
  );
}
...
export default function Phase3Page({ params }: { params: { projectId: string } }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const projects = JSON.parse(localStorage.getItem("wts_projects") ?? "[]");
      const p = projects.find((x: { id: string }) => x.id === projectId);
  ...
  async function runRoadmap() {
    ...
    if (!useMock) {
      try {
        for (let i = 0; i < 4; i++) {
          setAgent(i, "running"); await delay(500 + i * 200);
          setAgent(i, "done");
        }
    ...
    for (let i = 0; i < AGENTS.length; i++) {
      setAgent(i, "running"); await delay(800 + i * 300); setAgent(i, "done");
    }
          ...
          {view === "timeline" && (
            <div className={s.timeline}>
              <div className={s.timelineLabels}>
                {["1화","25화","50화","75화","100화"].map(l=><span key={l}>{l}</span>)}
              </div>
              <div className={s.timelineGrid}>
                {result.data.episodes.map(ep => {
                  ...
                  const isReversal = ep.flags.includes("reversal");
                  const isHook     = ep.flags.includes("hook");
                  return (
                    <div
                      key={ep.ep}
                      className={`${s.cell} ${isPeak?s.cellPeak:""} ${isReversal&&!isPeak?s.cellReversal:""}`}
                      style={{ background: arc.color + (isPeak?"cc":isHook?"55":"22") }}
                      title={`${ep.ep}화 — ${ep.title}\n${ep.summary}`}
                    >
                      {isPeak     && <span className={s.peakMark}>▲</span>}
                      {isReversal && !isPeak && <span className={s.revMark}>⚡</span>}
                    </div>
                  );
                })}
              </div>
              <div className={s.legend}>
                <span><span className={s.legPeak}>▲</span> 정점(PEAK)</span>
                <span><span className={s.legRev}>⚡</span> 반전</span>
                {result.data.arcs.map(arc => (
                  <span key={arc.num}><span className={s.legDot} style={{ background: arc.color }}/>{arc.num}막 {arc.name}</span>
                ))}
              </div>
            </div>
          )}
                      ...
                      {result.data.episodes.filter(e=>e.arc===arc.num).map(ep => (
                        <div key={ep.ep} className={`${s.epRow} ${ep.flags.includes("peak")?s.epPeak:""}`}>
                          ...
                          <div className={s.epFlags}>
                            ...
                            {ep.flags.includes("reversal") && <span className={s.tagRev}>REVERSAL</span>}
